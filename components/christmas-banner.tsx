...
export function ChristmasBanner() {
  ...
  const handleWatchVideo = async () => {
    setShowVideo(true)
    // No ocultar el banner, solo mostrar el video dentro
  }
        ...
        if (video) {
          ...
          const tryPlay = () => {
            if (video.readyState >= 3) {
              video.play().then(() => {
                console.log("Video reproduciéndose dentro del cartel")
              }).catch((err) => {
          ...
          {!showVideo ? (
            ...
            <>
              ...
              <button
                ...
                className="absolute top-2 right-2 z-20 bg-white/20 hover:bg-white/40 text-white rounded-full p-2 transition-colors"
